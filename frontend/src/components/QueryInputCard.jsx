...
export default function QueryInputCard({ prompt, setPrompt, onGenerate, isLoading }) {
  ...
  return (
    <div className="w-full bg-white border border-[#E5E7EB] rounded-3xl p-8 shadow-lg shadow-slate-200/50 relative overflow-hidden">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <span className="text-xs font-mono text-slate-400 font-semibold self-end md:self-auto bg-slate-105 px-2.5 py-1 rounded-md border border-[#E5E7EB]">
        ...
        <div className="relative group">
          <textarea
            ...
            className="w-full bg-slate-50 border border-[#E5E7EB] text-slate-850 rounded-2xl p-5 text-base font-sans leading-relaxed focus:outline-none focus:ring-4 focus:ring-indigo-100 focus:border-indigo-500 transition-all duration-300 resize-y disabled:opacity-60 disabled:cursor-not-allowed placeholder-slate-400 shadow-inner"
          />
        </div>
              ...
              <button
                ...
                className={`py-2 px-4 rounded-xl text-xs font-bold border transition-all duration-200 shadow-sm ${
                  prompt === item.prompt
                    ...
                    : "bg-slate-50 text-slate-655 border border-[#E5E7EB] hover:bg-slate-100 hover:text-slate-800"
          ...
          <button
            ...
            className={`w-full md:w-auto inline-flex items-center justify-center space-x-2.5 py-3 px-6 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-300 shadow-md ${
              isLoading || !prompt.trim()
                ...
                : "bg-[#2563EB] hover:bg-[#1D4ED8] hover:shadow-lg hover:shadow-blue-200/80 text-white border border-blue-500/10 active:scale-[0.98]"
            }`}
          >
            {isLoading ? (
              <>
